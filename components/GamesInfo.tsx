    ...
    <div className="w-full">
      <div className="p-2 text-white">
        ...
        <h1 className="text-4xl font-black mb-8 flex items-center justify-center gap-4 uppercase tracking-widest text-cyan-400">
          <Gamepad2 className="h-10 w-10 text-cyan-400" strokeWidth={2.5} />
          Video Games 
        </h1>
        ...
        <div className="mb-8 flex justify-between items-center max-w-7xl mx-auto px-4">
          <p className="text-sm text-gray-400">
            {searchTerm ? `Results for "${searchTerm}": ` : "Showing "}
            <span className="text-cyan-400 font-bold">{games.length}</span> of {totalGames}
          </p>

          <Link href="/games/crear">
            <button className="px-6 py-2 rounded-xl bg-gradient-to-r from-fuchsia-500 to-cyan-400 text-black font-bold hover:scale-110 transition shadow-[0_0_15px_rgba(6,182,212,0.3)] uppercase text-xs">
              + New Game
            </button>
          </Link>
        </div>
        ...
        <div className="grid gap-8 sm:grid-cols-2 lg:grid-cols-4 max-w-7xl mx-auto px-4">
          {!hasGames ? (
            <div className="col-span-full text-center py-24 bg-white/5 border border-white/10 rounded-3xl">
              <p className="text-2xl text-gray-400 font-bold">No games found 😢</p>
            </div>
          ) : (
            games.map((game: any) => (
              <div
              ...
              >
                ...
                <div className="p-5 flex flex-col">
                  <h2 className="font-black text-lg text-white group-hover:text-cyan-400 transition uppercase tracking-tighter truncate">
                    {game.title}
                  </h2>
                  <span className="text-xs text-gray-400 mt-1 mb-4 uppercase font-bold tracking-widest">
                    {game[consoleModelName]?.name || "No console assigned"}
                  </span>
